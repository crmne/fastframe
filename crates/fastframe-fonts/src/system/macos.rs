...
//!
//! Probing every font on the machine cannot be made right here, for two
//! reasons a scan cannot see:
//!
...
//!   beside dozens of optional download faces that nothing in the asset
//!   metadata separates it from. Scanning that directory lets those in, and a
//!   Japanese or Korean desktop then draws its interface in a Taiwanese or a
//!   brush face.
//! * A face declares the regions it covers in its OS/2 code pages, and may
//!   declare all four while being drawn for one.
//!
...
//! draws with. Extracted from Spotifast's `mac_fonts.rs`.

#![allow(
    unsafe_code,
    reason = "CoreText and Core Foundation are C interfaces without a safe binding in the apps' trees"
)]
...
use super::{Found, SCRIPTS, draws, faces, map};
...
pub(super) fn load() -> Vec<Found> {
    let started = std::time::Instant::now();
    let mut found: Vec<Found> = Vec::new();
    let mut taken: Vec<(PathBuf, u32)> = Vec::new();
    for (script, probe, _) in SCRIPTS {
...
fn face_index(path: &std::path::Path, family: &str, probe: char) -> Option<u32> {
    ...
    faces(&data)
        ...
        .min_by(|(_, one), (_, other)| {
            let distance =
                |font: &skrifa::FontRef<'_>| (font.attributes().weight.value() - 400.0).abs();
...
fn names(font: &skrifa::FontRef<'_>, family: &str) -> bool {
    [
...
mod tests {
    ...
    fn every_script_resolves_to_a_readable_face() {
        ...
        for font in &fonts {
            assert!(!font.bytes.is_empty(), "{} is empty", font.script);
            assert!(
                font.index < super::super::MAX_FACES,
